...
app.get('/jokes/:id' , (req,res)=>{
  ...
  const desiredJoke = jokes.find((joke)=> joke.id === id)
  res.json(desiredJoke)
})
...
app.get('/filter' , (req,res)=>{
  ...
  const desiredJoke = jokes.filter((joke)=> joke.jokeType === type)
  console.log()
  res.json(desiredJoke)
})
...
app.delete('/jokes/:id', (req,res)=> {
  ...
  if(jokeIndex != -1) {
    ...
    res.status(200).json("joke deleted ")
  } else {
...
app.delete('/all', (req,res)=>{
  if (masterKey === req.query.key) {
    jokes=[]
    res.status(200).json("All jokes have been deleted ")
  } else {
    res.status(400).json("No work ")
    console.log(console.error({message: "no work"}))
  }
})
